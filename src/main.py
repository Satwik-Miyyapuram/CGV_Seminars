...
def main():
    ...
    parser.add_argument("--data", type=str, default="nerf_synthetic/chair", help="Path to the dataset directory")
    parser.add_argument("--dataparser", type=str, default="auto", choices=["auto", "blender", "colmap", "nerfstudio"], help="Dataparser type (default: auto-detect)")
    parser.add_argument("--downscale-factor", type=int, default=None, help="Downscale factor for images")
    parser.add_argument("--max-num-iterations", type=int, default=30000, help="Max number of iterations")
    parser.add_argument("--background-color", type=str, default="auto", choices=["auto", "white", "black", "random"], help="Background color for evaluation rendering")
    parser.add_argument("--use-real-scene", type=str, default="auto", choices=["auto", "true", "false"], help="Whether to use real-world scene parameters")
    parser.add_argument("--load-dir", type=str, default=None, help="Path to checkpoint directory to load")
    ...
    if dataparser_type == "auto":
        if (data_path / "sparse").exists() and not (data_path / "transforms.json").exists():
            ...
            print("Auto-detected Nerfstudio preprocessed dataset structure. Using NerfstudioDataParser.")
    ...
    if dataparser_type == "blender":
        ...
        print(f"Configured Blender dataparser. Scene properties: background_color={bg_color}, use_real_scene={use_real}")
        
    elif dataparser_type == "colmap":
        from nerfstudio.data.dataparsers.colmap_dataparser import ColmapDataParserConfig
        colmap_path = Path("sparse/0") if (data_path / "sparse/0").exists() else Path("colmap/sparse/0")
        ...
        print(f"Configured COLMAP dataparser (colmap_path={colmap_path}). Scene properties: background_color={bg_color}, use_real_scene={use_real}")
        ...
        bg_color = "random" if args.background_color == "auto" else args.background_color
        use_real = True if args.use_real_scene == "auto" else (args.use_real_scene == "true")
        print(f"Configured Nerfstudio dataparser. Scene properties: background_color={bg_color}, use_real_scene={use_real}")
