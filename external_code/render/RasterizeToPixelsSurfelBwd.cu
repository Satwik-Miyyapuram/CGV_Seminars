...
__global__ void rasterize_to_pixels_surfel_bwd_kernel(
...
#pragma unroll
        ...
        for (uint32_t t = max(0, batch_end - warp_bin_final); t < batch_size;
            ...
            if (valid) {
                ...
                // gaussian throw out
                if (sigma < 0.f || vis < ALPHA_THRESHOLD || alpha < ALPHA_THRESHOLD) {
                    valid = false;
                }
            }
...
void launch_rasterize_to_pixels_surfel_bwd_kernel(
    ...
    if (cudaFuncSetAttribute(
            rasterize_to_pixels_surfel_bwd_kernel<CDIM, float>,
...
#define __INS__(CDIM)                                                          \
    template void launch_rasterize_to_pixels_surfel_bwd_kernel<CDIM>(          \
